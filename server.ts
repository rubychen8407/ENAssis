...
async function fetchYouTubeCaptions(
  ...
  try {
    const listRes = await fetch(`https://video.google.com/timedtext?type=list&v=${youtubeVideoId}`);
    ...
    const tracks = trackMatches.map((m) => {
      ...
      const get = (key: string) => {
        const am = attrs.match(new RegExp(`${key}="([^"]*)"`));
        return am ? am[1] : '';
      };
      return { langCode: get('lang_code'), kind: get('kind') };
    });
    if (tracks.length === 0) return null;

    // Prefer a manually-authored English track, then any English track (incl. auto-generated).
    // Non-English-only videos are skipped for now to keep transcript quality/accuracy high.
    const chosen =
      tracks.find((t) => t.langCode.startsWith('en') && t.kind !== 'asr') ||
      tracks.find((t) => t.langCode.startsWith('en'));
    if (!chosen) return null;

    const capUrl = `https://video.google.com/timedtext?lang=${chosen.langCode}&v=${youtubeVideoId}${
    ...
    return { transcript, isAutoGenerated: chosen.kind === 'asr', languageCode: chosen.langCode };
  } catch (err: any) {
